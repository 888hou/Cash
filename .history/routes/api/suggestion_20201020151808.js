...
router.post("/add",(req,res)=>{
    ...
    new Suggestion(suggestionFields).save().then(suggestion => {
        ...
        var sms_contents = qs.stringify({
            content:"收到来自："+suggestionFields.mobile+"，"+suggestionFields.name+"；提出的投诉建议。内容："+suggestionFields.content,
            mobile:config.kefuMobile,
            token:'qhdfydx'
        });
        var sms_options = {
...
router.post("/edit/:id",passport.authenticate("jwt",{session:false}),(req,res)=>{
    ...
    Suggestion.findOneAndUpdate(
        ...
        {new:true}
            ...
            //发送短信
            if(suggestion.status == "完成"){
                //发送短信
                var sms_contents = qs.stringify({
                    content:"您的投诉建议，客户服务中心已回复，请使用公众号查看详情："+suggestion.feedback,
