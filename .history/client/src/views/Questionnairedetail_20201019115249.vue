<template>
    <div class="container">
        ...
        <div class="questions">
            <div v-for="(value, index) in questionData" :key="index">
                {{value.order}}-{{value.name}}
                <el-button class="btnAdd" type="primary" size="small" icon="view" v-if="user.identity == 'manager'" @click="onEdit(value)">修改题目</el-button>
            </div>
        </div>
        <el-button class="btnAdd" type="primary" size="small" icon="view" v-if="user.identity == 'manager'" @click="onAdd()">添加题目</el-button>
