<template>
    <div class="container">
        ...
        <div class="questions" v-for="(value, index) in questionData" :key="index">
            <p>{{value.order}}、{{value.name}}<span v-if="value.type == '填空题' || value.type == '问答题'">({{value.remark}})</span></p>
            <div class="answers" v-if="value.type == '填空题'">
                <el-input class="answer" :placeholder="value.remark" ></el-input>
            </div>
            <div class="answers" v-if="value.type == '问答题'">
                <el-input class="answer" type="textarea" :placeholder="value.remark"></el-input>
            </div>
            <div class="answers" v-if="value.type == '单选题'">
                <el-radio class="answer" v-for="(value, index) in questionData.answer" :key="index" label="value">{{value}}</el-radio>
            </div>
