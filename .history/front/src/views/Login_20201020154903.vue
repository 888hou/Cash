...
export default {
    ...
    methods: {
      onSubmit(values) {
        ...
        this.$axios.post("/api/wusers/login",this.loginUser)
        .then((res) => {
          console.log(res);
      ...
      if(!this.isEmpty(this.$route.params.openid)){
        ...
        .then((res) => {
          ...
          if(!this.isEmpty(this.$route.params.route)){
            if(this.isEmpty(this.$route.params.action)){
              this.$router.push("/"+this.$route.params.route);
            }else{
              this.$router.push("/"+this.$route.params.route+"/"+this.$route.params.action);
            }
          }else{
            this.$router.push("/index");
          }
