        ...
        <div class="questions" v-for="(value, index) in questionData" :key="index">
            ...
            <div class="answers" v-if="value.type == '填空题'">
                <van-field
                  border
                  class="answer"
                  v-model="questionData[index].input"
                  :placeholder="value.remark"
                  :rules="[{ required: (value.require == '必填')?true:false, message: '请填写'}]"
                />
            </div>
            ...
            <div class="answers" v-if="value.type == '单选题'">
              <van-field :rules="[{ required: (value.require == '必填')?true:false, message: '请选择'}]">
                <template #input>
                  <van-radio-group v-model="questionData[index].input">
                    <van-radio border class="answer" v-for="(value, index) in value.answer" :key="index" :name="value">{{value}}</van-radio>
                  </van-radio-group>
                </template>
              </van-field>
            ...
            <div class="answers" v-if="value.type == '多选题'">
              <van-field :rules="[{ required: (value.require == '必填')?true:false, message: '请选择'}]">
                <template #input>
                  <van-checkbox-group -group v-model="questionData[index].input">
                    <van-checkbox border class="answer" v-for="(value, index) in value.answer" :key="index" :name="value">{{value}}</van-checkbox>
                  </van-checkbox-group>
                </template>
              </van-field>
